...
def ascii_clean(text):
    # Map all non-ascii characters to ascii equivalents
    replacements = {
        '🟢': '[SAFE]',
        '🟡': '[REVIEW]',
        '🔴': '[FRAUD]',
...
def clean_currency(text):
    # Replace currency dollar signs like $48 billion, $65 billion, $500, $10,000, $14,950
    # $ followed by digits
    text = re.sub(r'\$(\d[\d,]*(?:\.\d+)?(?:\s*(?:billion|million|k|M|B))?)', r'\\\$\1', text)
    return text

def format_text_segment(text):
    # Markdown formatting
    text = re.sub(r'\*\*(.*?)\*\*', r'\\textbf{\1}', text)
    text = re.sub(r'\*([^*]+)\*', r'\\textit{\1}', text)
    # Inline code
    def clean_code(m):
        code_str = m.group(1).replace('_', r'\_').replace('&', r'\&').replace('%', r'\%').replace('#', r'\#')
        return r'\texttt{' + code_str + '}'
    text = re.sub(r'`([^`]+)`', clean_code, text)

    # Escape raw special characters
    tokens = re.split(r'(\\(?:textbf|textit|texttt|ref|cite|label|vspace|hspace|safeincludegraphics|S)\{[^}]*\})', text)
    res = []
    for tok in tokens:
        if tok.startswith('\\'):
            # If textbf or textit, ensure special chars inside are escaped
            if tok.startswith(r'\textbf{') or tok.startswith(r'\textit{'):
                cmd = tok[:8]
                inside = tok[8:-1]
                inside = inside.replace('&', r'\&').replace('%', r'\%').replace('#', r'\#').replace('_', r'\_')
                res.append(cmd + inside + '}')
            else:
                res.append(tok)
        else:
            t = tok
...
def format_line(line):
    ...
    for part in parts:
        if part.startswith('$') and part.endswith('$') and len(part) > 1:
            ...
            out.append(format_text_segment(part))
    return "".join(out)
...
def build():
    ...
    md = clean_currency(md)

    # 3. ASCII cleaning
    md = ascii_clean(md)

    # 4. Student roll numbers
...
'''
    ...
    while idx_line < len(lines):
        ...
        if sline.startswith("$$"):
            ...
            # Clean math block: filter empty lines
            cleaned_math = " ".join([ml for ml in math_lines if ml])
            cleaned_math = re.sub(r'(?<!\\)%', r'\%', cleaned_math)
            # Ensure scale_pos_weight inside math has scale\_pos\_weight
            cleaned_math = cleaned_math.replace(r'\text{scale_pos_weight}', r'\text{scale\_pos\_weight}')
            cleaned_math = cleaned_math.replace(r'\text{merch_lat}', r'\text{merch\_lat}')
            cleaned_math = cleaned_math.replace(r'\text{merch_long}', r'\text{merch\_long}')
        ...
        if sline.startswith("# Chapter"):
            ...
            latex_out.append(r"\chapter{" + format_line(t_str) + "}")
            idx_line += 1
            continue
        ...
        if sline.startswith("# Appendix"):
            ...
            latex_out.append(r"\chapter*{" + format_line(app_title) + r"}\addcontentsline{toc}{chapter}{" + format_line(app_title) + "}")
            idx_line += 1
            continue
        ...
        if sline.startswith("## "):
            ...
            latex_out.append(r"\section{" + format_line(h2_clean) + "}")
...
\end{figure}''')
        ...
        if sline.startswith("### "):
            ...
            latex_out.append(r"\subsection{" + format_line(h3_clean) + "}")
...
\end{figure}''')
        ...
        if sline.startswith("#### "):
            ...
            latex_out.append(r"\subsubsection{" + format_line(h4_clean) + "}")
            idx_line += 1
            continue
        ...
        if sline.startswith("- ") or sline.startswith("* "):
            ...
            if not in_itemize:
                ...
                in_itemize = True
            latex_out.append(r"\item " + format_line(sline[2:]))
            idx_line += 1
            continue
        ...
        if m_num:
            ...
            if not in_enumerate:
                ...
                in_enumerate = True
            latex_out.append(r"\item " + format_line(m_num.group(2)))
            idx_line += 1
            continue

        # Regular paragraph
        latex_out.extend(end_lists())
        latex_out.append(format_line(sline) + "\n")
        idx_line += 1
    ...
    print(f"SUCCESS! Flawless LaTeX written to: {tex_path}")
