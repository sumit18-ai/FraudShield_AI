...
def escape_text(text):
    # Escape markdown formatting to LaTeX
    # Handle inline math by splitting on $
    segments = re.split(r'(\$[^\$]+\$)', text)
    result = []
    for seg in segments:
        if seg.startswith('$') and seg.endswith('$') and len(seg) > 1:
            # Clean math segment
            clean_math = seg.replace('&', r'\&').replace('%', r'\%')
            result.append(clean_math)
        else:
            s = seg
            ...
            # Escape LaTeX specials (excluding backslash used for commands)
            s = s.replace('&', r'\&')
            s = s.replace('%', r'\%')
            s = s.replace('#', r'\#')
            s = s.replace('_', r'\_')
...
def main():
    ...
    # Pre-clean student roll numbers
...
\begin{center}
    ...
    {\normalsize Assistant Professor}\\[1.0cm]
    
    \includegraphics[width=2.4in]{figures/fig4_architecture.jpg}\\[1.0cm]
...
'''
    ...
    def flush_table(tb):
        ...
        if num_cols == 0:
            return ""
        col_spec = " | ".join(["X" for _ in range(num_cols)])
        res = []
        res.append(r"\begin{table}[H]")
        res.append(r"\centering")
        res.append(r"\footnotesize")
        res.append(r"\begin{tabularx}{\textwidth}{| " + col_spec + r" |}")
        res.append(r"\hline")
        res.append(r"\rowcolor{tableheader} " + " & ".join([r"\textbf{" + escape_text(h) + "}" for h in headers]) + r" \\ \hline")
        for idx, r_data in enumerate(data_rows):
            while len(r_data) < num_cols:
                r_data.append("")
            clean_cells = [escape_text(c) for c in r_data[:num_cols]]
    ...
    for line in lines:
        ...
        if sline.startswith("# Chapter"):
            ...
            latex_out.append(r"\chapter{" + escape_text(t_str) + "}")
            continue
        ...
        if sline.startswith("# Appendix"):
            ...
            latex_out.append(r"\chapter*{" + escape_text(app_title) + r"}\addcontentsline{toc}{chapter}{" + escape_text(app_title) + "}")
            continue
        ...
        if sline.startswith("## "):
            ...
            latex_out.append(r"\section{" + escape_text(h2_clean) + "}")

            if "High-Level System Architecture" in h2 or "2.3.2" in h2:
                latex_out.append(r'''\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig4_architecture.jpg}
...
\end{figure}''')
        ...
        if sline.startswith("### "):
            ...
            latex_out.append(r"\subsection{" + escape_text(h3_clean) + "}")

            if "Hybrid Stacking Ensemble" in h3 or "4.2.2" in h3:
                latex_out.append(r'''\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig15_stacking.jpg}
...
\end{figure}''')
                ...
                latex_out.append(r'''\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig17_treeshap.jpg}
...
\end{figure}''')
                ...
                latex_out.append(r'''\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig18_mule_ring.jpg}
...
\end{figure}''')
        ...
        if sline.startswith("#### "):
            ...
            latex_out.append(r"\subsubsection{" + escape_text(h4_clean) + "}")
            continue
        ...
        if sline.startswith("- ") or sline.startswith("* "):
            ...
            if not in_itemize:
                ...
                in_itemize = True
            latex_out.append(r"\item " + escape_text(sline[2:]))
            continue
        ...
        if m_num:
            ...
            if not in_enumerate:
                ...
                in_enumerate = True
            latex_out.append(r"\item " + escape_text(m_num.group(2)))
            continue

        # If regular paragraph, close open list environments
        latex_out.extend(end_lists())

        # Check for display math $$...$$
        if sline.startswith("$$") and sline.endswith("$$") and len(sline) > 4:
            clean_disp_math = sline[2:-2].strip().replace('&', r'\&').replace('%', r'\%')
            latex_out.append(r"\begin{equation}")
            latex_out.append(clean_disp_math)
            latex_out.append(r"\end{equation}")
            continue

        # Regular paragraph
        latex_out.append(escape_text(sline) + "\n")

    # Clean up any trailing open environments
    latex_out.extend(end_lists())
    if in_table:
        latex_out.append(flush_table(table_buf))
    ...
    print(f"SUCCESS! Complete LaTeX generated at: {tex_path}")
    print(f"Total lines: {len(latex_out)}, Total characters: {len(full_tex)}")
